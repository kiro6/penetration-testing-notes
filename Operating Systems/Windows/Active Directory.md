...
- **It is a challenge-response authentication protocol and uses three messages to authenticate:**
  1. **NEGOTIATE_MESSAGE:** The client sends a message to the server indicating its intention to authenticate using NTLM. This message typically includes information about the client's capabilities and preferences.
  2. **CHALLENGE_MESSAGE:** Upon receiving the NEGOTIATE_MESSAGE, the server responds with a challenge message. This challenge is typically a random string of characters generated by the server. The purpose of the challenge is to verify that the client possesses the necessary credentials to authenticate.
  3. **AUTHENTICATE_MESSAGE:** The client receives the challenge from the server and generates a response known as the AUTHENTICATE_MESSAGE. This message is created by encrypting the challenge using the client's credentials (typically, the user's password hash). The encrypted response is sent back to the server.
  4. Upon receiving the AUTHENTICATE_MESSAGE, the server decrypts the response using the stored credentials for the user attempting to authenticate. If the decrypted response matches the challenge sent by the server, authentication is successful, and the client is granted access.
